...
use App\Models\Page;
use App\Models\User;
use App\Traits\HasMinistereScope;
use Illuminate\Http\Request;

class DashboardController extends Controller
{
    use HasMinistereScope;
    ...
    public function ministry(Request $request)
    {
        ...
        $articleStats = Article::where('ministere_id', $ministereId)
            ->selectRaw('
                ...
                SUM(CASE WHEN statut = "brouillon" THEN 1 ELSE 0 END) as brouillons,
                SUM(CASE WHEN en_avant = 1 THEN 1 ELSE 0 END) as en_avant,
                COALESCE(SUM(vues), 0) as vues_totales
            ')->first();
        ...
        $evenementStats = Evenement::where('ministere_id', $ministereId)
            ->selectRaw('
                COUNT(*) as total,
                SUM(CASE WHEN statut = "a_venir" THEN 1 ELSE 0 END) as a_venir,
                SUM(CASE WHEN statut = "en_cours" THEN 1 ELSE 0 END) as en_cours,
                SUM(CASE WHEN categorie = "recurrent" THEN 1 ELSE 0 END) as recurrents,
                SUM(CASE WHEN categorie = "permanent" THEN 1 ELSE 0 END) as permanents
            ')->first();
        ...
        $mediaStats = Media::where('ministere_id', $ministereId)
            ->selectRaw('
                ...
                ROUND(SUM(taille) / 1024 / 1024, 2) as taille_totale_mb
            ')->first();
        ...
        $messageStats = MessageContact::where('ministere_id', $ministereId)
            ->selectRaw('
                ...
                SUM(CASE WHEN statut = "repondu" THEN 1 ELSE 0 END) as respondues
            ')->first();
            ...
            'data'    => [
                ...
                'stats'                => [
                    'pages' => [
                        ...
                        'publiees'   => $pageStats->publiees,
                        'brouillons' => $pageStats->brouillons,
                    ],
                    'articles' => [
                        'total'        => $articleStats->total,
                        'publies'      => $articleStats->publies,
                        'brouillons'   => $articleStats->brouillons,
                        'vues_totales' => $articleStats->vues_totales,
                        'en_avant'    => $articleStats->en_avant,
                        'par_type'    => $articleParType,
                    ],
                    'evenements' => [
                        'total'      => $evenementStats->total,
                        'a_venir'   => $evenementStats->a_venir,
                        'en_cours'  => $evenementStats->en_cours,
                        'recurrents'=> $evenementStats->recurrents,
                        'permanents'=> $evenementStats->permanents,
                    ],
                    'medias' => [
                        ...
                        'taille_totale_mb'=> $mediaStats->taille_totale_mb,
                    ],
                    'messages' => [
                        'total'    => $messageStats->total,
                        'non_lus' => $messageStats->non_lus,
                        'lus'     => $messageStats->lus,
                        'repondus'=> $messageStats->respondues,
                    ],
                ],
    ...
    public function statsEngagement(Request $request)
    {
        ...
        $tauxReponse = $messageStats->total > 0
            ? round(($messageStats->respondues / $messageStats->total) * 100, 1)
            : 0;
